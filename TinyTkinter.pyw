...
def changeSetting(row=1):
	fieldName, idControl, _, _ = getInformation();
...
def run():
	fieldName, _, _, _ = getInformation();
...
def addLabel():
	...
	if(fieldName):
		ranNumber = createRandomNumber() # Generates a random 4-digit number
		hashtag = "#Script\n"
		item = f"""title_{ranNumber} = Label(root, text="new item").grid(row={idControl},column=0, columnspan=1, padx=0,pady=0) #ID_{idControl}\n\n"""
...
def addField():
	...
	if(fieldName):
		ranNumber = createRandomNumber() # Generates a random 4-digit number
		hashtag = "#Script\n"
		item = f"""fieldName_{ranNumber} = Entry(root, width=50, borderwidth=2) #ID_{idControl}\nfieldName_{ranNumber}.grid(row={idControl},column=0, columnspan=2, padx=7,pady=2) #ID_{idControl}\n#ID_{idControl}\n\n"""
		lines = readFile(fieldName)
		searchAndWriteFile(fieldName, hashtag, lines, item) # search hashtag in app_123456.py and add item before hashtag
		changeSetting()
	else:
...
def addButton():
	...
	if(fieldName):
		...
		item2 = f"""Button_{idControl}_{ranNumber} = Button(root, text=f"Button{idControl}", padx=40, command=btn_{idControl}_{ranNumber}).grid(row={idControl}, column=0) #ID_{idControl}\n\n"""
